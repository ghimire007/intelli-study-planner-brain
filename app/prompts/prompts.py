...
Analyze the handbook and return all valid elective subjects.
Filter out subjects in Core, in chosen Major/s, and already in student record.

LIST THE REMAINING SUBJECTS IN ORDER OF RELEVANCE TO THE CHOSEN DEGREE, with most relevant being listed first. 
...
Analyze SOLS record against the handbook to return all missing subjects for student's degree core and major/s.
...
Your job is to ensure that the built semester-by-semester study plan is valid, satisfying all degree requirements for the student's confirmed degree metadata. Do NOT assume any default course. Use only injected handbook data and confirmed metadata. If degree metadata changes, call `confirm_metadata_tool` and re-fetch the handbook.
