...
Extract the following three fields from the UOW SOLS enrolment record OR student given information and return ONLY a JSON object — no explanation, no markdown, no code block.
...
- year is the year given or the earliest year found in the subject list (the student's commencement year). Return null if it cannot be determined.
- campus is the canonical campus name — map as follows:
  ...
  Return null if campus cannot be determined.
""".strip()
